...
// Ver.0.1.15: 徐々にHP回復「そよぎの輪」を廃止し、移動速度アップ「追い風」へ置換。
if(!progress.arpgYunoRegenToHaste015){
  const old=progress.yunoSkills.regen||0;
  if(old>0){progress.yunoSP=(progress.yunoSP||0)+(old>=3?5:old>=2?3:2);progress.yunoSkills.regen=0;}
  progress.yunoSkills.haste=Math.max(progress.yunoSkills.haste||0,1);
  progress.arpgYunoRegenToHaste015=true;
}

// Ver.0.1.15: 追い風/風渡りを廃止し、主人公MP回復の能動スキルへ戻す。
if(!progress.arpgYunoMPReturn014){const wf=progress.yunoSkills.windFlow||0;if(wf>0){progress.yunoSP=(progress.yunoSP||0)+(wf>=2?5:2);}progress.yunoSkills.windFlow=0;progress.arpgYunoMPReturn014=true;}
...
if(!progress.gyouSkills)progress.gyouSkills={cover:0,taunt:0,doubleThrust:0,shieldBash:0};
if(progress.gyouSkills.shieldBash===undefined)progress.gyouSkills.shieldBash=0;
progress.gyouSkills.cover=Math.min(3,progress.gyouSkills.cover||0);
progress.gyouSkills.taunt=Math.min(3,progress.gyouSkills.taunt||0);
progress.gyouSkills.doubleThrust=Math.min(2,progress.gyouSkills.doubleThrust||0);
progress.gyouSkills.shieldBash=Math.min(3,progress.gyouSkills.shieldBash||0);
...
if(!progress.arpgSkillCleanup015){
  // 追い風はVer.0.1.15でARPG用支援として復活。
  const sCounter=progress.suzuSkills?.counter||0;
  if(sCounter>0){progress.suzuSP=(progress.suzuSP||0)+3;progress.suzuSkills.counter=0;progress.suzuSpentSP=Math.max(0,(progress.suzuSpentSP||0)-3);}
  const gBreath=progress.gyouSkills?.earthBreath||0;
  if(gBreath>0){progress.gyouSP=(progress.gyouSP||0)+(gBreath>=2?5:2);progress.gyouSkills.earthBreath=0;}
  progress.arpgSkillCleanup015=true;
}

// Ver.0.1.11: ジュウのターン制向け旧スキルを整理し、使用済みSPを一度だけ返却。
if(!progress.arpgGyouCleanup011){
  const refundTable={fortify:2,manaGuard:3,healGuard:3,counter:3};
  let refund=0;
  for(const [k,c] of Object.entries(refundTable)){if((progress.gyouSkills?.[k]||0)>0){refund+=c;progress.gyouSkills[k]=0;}}
  if(refund>0)progress.gyouSP=(progress.gyouSP||0)+refund;
  progress.arpgGyouCleanup011=true;
}

function yunoSkillCost(key,lv){
  const table={
    heal:[2,1,2], regen:[2,1,2], wind:[2,1,2],
    haste:[2,3], mpRegenAll:[2,3], archery:[2,3], windFlow:[2,3]
  };
  const arr=table[key]||[3];
  return arr[Math.min(lv,arr.length-1)];
}
function yunoSkillMax(key){
  return ({heal:3,regen:0,wind:3,haste:2,mpRegenAll:2,archery:2,windFlow:0})[key]||1;
}
function gyouSkillMax(key){
  return ({cover:3,taunt:3,doubleThrust:2,shieldBash:3})[key]||1;
}
function gyouSkillCost(key,lv=0){
  const table={cover:[2,3,4],taunt:[2,3,4],doubleThrust:[3,4],shieldBash:[3,4,5]};
  const arr=table[key]||[3];
  return arr[Math.min(lv,arr.length-1)];
}
...
function drawTitle(){
  ...
  let tx=480-totalW/2;
  for(const [part,col] of [[titleLeft,'#fff'],[titleA,'#ff9a3d'],[titleRight,'#fff']]){
  ...
  labels.forEach((lab,i)=>{
    if(i===1)return; // リセットは誤タップ防止のため独立した小ボタンにする
  ...
  // 左下の小さなリセットボタン。押したあとにも確認を出す。
  outlineRect(34,478,188,28,titleSelection===1?'#f7e8e4':'rgba(40,35,40,.62)','#b8776c',1.5);
  text('初期状態へリセット',128,492,11,'center',titleSelection===1?'#6b2b24':'#f2d9d4',800);
  ...
  if(battle.turn==='player'){
    if(battleMenu==='target'){
      ...
      if(isGyouTurn()){
        ...
        const opts=[
          ['cover','かばう','(5)'],['taunt','挑発','(4)'],['doubleThrust','二段突き','(6)'],['shieldBash','シールドバッシュ','(7)'],
          ['grandGuard','大守護','(16)']
        ];
        opts.forEach((o,i)=>{
        ...
        const yu=[
          ...
          ['haste','疾風','(8)'],['archery','二連射','(7)'],['mpRegenAll','魔力の風','(12)']
        ];
        yu.forEach((o,i)=>{
        ...
        text('MP自然回復（常時）',840,404,(progress.heroManaSkill||0)?15:14,'center',(progress.heroManaSkill||0)?'#102b40':'#ffffff',900);
...
function yunoAction(mode,target='hero'){
  ...
  if(mode==='healAll'){
    ...
    battleMessage='「そよぎの輪」はARPG版では廃止されました。';
    ...
    sfx('wind');
    const slv=Math.max(1,progress.yunoSkills?.mpRegenAll||1),amount=slv>=2?26:16;
    const before=battle.heroMP;battle.heroMP=Math.min(progress.maxMP,battle.heroMP+amount);
    battleMessage=`ユーノの「魔力の風」！ ${heroName}のMPが${battle.heroMP-before}回復！`;setBattleFx('heal',360,255);
  }
...
function gyouAction(mode,target='hero'){
  ...
  ensureGyouBattle();
  const key={cover:'cover',taunt:'taunt',doubleThrust:'doubleThrust',shieldBash:'shieldBash'}[mode];
  ...
  const cost={cover:5,taunt:4,doubleThrust:6,shieldBash:7,grandGuard:16}[mode];
  ...
  if(mode==='fortify'){sfx('earth');battle.gyouDefTurns=3;battleMessage='ジュウの「岩守り」！ 防御力が大きく上がった！';}
    ...
    sfx('spear');const gs=gyouStats(),lv=progress.gyouSkills.doubleThrust||1,hits=lv>=2?3:2,parts=[];
    for(let i=0;i<hits;i++){const d=Math.floor(gs.atk*(lv>=2?.80:.72))+(lv>=2?7:5)+Math.floor(Math.random()*4);parts.push(d);damageEnemy(d);if(enemiesDefeated())break;}
    battleMessage=`ジュウの「${lv>=2?'三段突き':'二段突き'}」！ ${parts.join('＋')}ダメージ！`;setBattleFx('slash');if(enemiesDefeated()){battle.turn='win';battleCooldown=1;return;}
  }
  else if(mode==='shieldBash'){sfx('guard');const d=Math.floor(gyouStats().def*.65)+18+(progress.gyouSkills.shieldBash||1)*8;damageEnemy(d);battleMessage=`ジュウの「シールドバッシュ」！ ${d}ダメージ＋大きく吹き飛ばした！`;setBattleFx('slash');if(enemiesDefeated()){battle.turn='win';battleCooldown=1;return;}}
...
function drawMenu(){
  ...
  if(menuPage==='status'){
    ...
    if(menuCharacter==='gyou' && gyouEnabled){
      ...
      const gsks=[
        ['cover','かばう','主人公の近くで攻撃を受け止める'],
        ['taunt','挑発','周囲の敵をジュウへ引きつける'],
        ['doubleThrust','二段突き','Lv.2で三段突きに強化'],
        ['shieldBash','シールドバッシュ','盾で吹き飛ばす / 大ノックバック']
      ];
      gsks.forEach((sk,i)=>{
        const key=sk[0],col=i%2,row=Math.floor(i/2),x=85+col*410,y=245+row*105,lv=progress.gyouSkills[key]||0,max=gyouSkillMax(key),cost=gyouSkillCost(key,lv);
        outlineRect(x,y,370,88,'#3c3b2d','#999064',2);
        const name=key==='doubleThrust'&&lv>=2?'三段突き':sk[1];
        text(name+(lv>0?` Lv.${lv}`:''),x+14,y+27,17,'left','#f4efcf',800);
        text(sk[2],x+14,y+53,12,'left','#d7d1ae');
        text(lv>=max?`Lv.${lv} 最大`:(lv?`次 Lv.${lv+1} SP${cost}`:`習得 SP${cost}`),x+350,y+76,11,'right',lv>=max?'#b9d29e':'#ffe4a0');
      });
      text('強化：かばう＝持続/CT　挑発＝持続/範囲　二段突き＝3HIT　バッシュ＝威力/吹飛距離',480,468,12,'center','#d9d3ad');
      if(progress.hiddenSkillsUnlocked){outlineRect(710,485,180,50,progress.hiddenSkills?.gyou?'#ece8ce':'#2e3340','#999064',2);text('天地崩槍',800,505,15,'center',progress.hiddenSkills?.gyou?'#494427':'#fff',900);text(progress.hiddenSkills?.gyou?'習得済み':'SP100',800,526,12,'center','#ffe7a5');}
        }else if(menuCharacter==='yuno' && yunoEnabled){
      text(`ユーノ SP：${progress.yunoSP||0}`,55,215,18,'left','#d8fff5');
      const ysks=[
        ['heal','風の癒し','主人公HP回復'],['haste','追い風','主人公・仲間の移動速度アップ'],
        ['wind','風刃嵐','敵全体攻撃'],
        ['mpRegenAll','魔力の風','主人公のMPを直接回復'],['archery','二連射','Lv2で風纏三連射']
      ];
      ysks.forEach((s,i)=>{
        const key=s[0],col=i%4,row=Math.floor(i/4),x=35+col*225,y=245+row*105,lv=progress.yunoSkills[key]||0,max=yunoSkillMax(key);
        outlineRect(x,y,210,88,'#183a43','#59aaa6',2);
        text(s[1],x+12,y+25,15,'left','#d8fff5',800);
      ...
      {
        const fl=progress.suzuSkills?.fightingFlame||0;
        outlineRect(285,365,390,70,fl?'#ffe7d1':'#3b4653',fl?'#e08a4e':'#7c8790',2);
        text('闘炎（パッシブ）',310,390,18,'left',fl?'#6b2d1e':'#d9dde0',800);
        text(fl>=2?'ARPG：炎スキル威力 +12%':fl===1?'ARPG：炎スキル威力 +6%':'火炎斬り・範囲炎魔法を強化',310,414,12,'left',fl?'#8d4a3b':'#aab0b5');
        text(fl>=2?'Lv.2 最大':fl===1?'次 Lv.2 SP3':'習得 SP2',650,414,11,'right',fl?'#8a4931':'#ffe7a5',800);
      }
      text('反撃スキル「炎返し」はARPG化に合わせて廃止しました。',480,470,14,'center','#ffffff');
    }else{
      ...
      text('水脈循環（パッシブ）',705,386,17,'left','#17324a',800);
      text(hm>=2?'MP自然回復量 +100%':hm===1?'MP自然回復量 +50%':'常時MP自然回復を強化',705,410,12,'left','#3d5d73');
...
function menuTap(x,y){
  ...
  if(menuPage==='skill'&&progress.hiddenSkillsUnlocked&&y>=445&&y<=540){
  ...
  if(menuPage==='skill' && menuCharacter==='gyou' && gyouJoinConfirmed && y>=245 && y<=438){
    const col=Math.floor((x-85)/410),row=Math.floor((y-245)/105);
    if(col>=0&&col<2&&row>=0&&row<2){
      const keys=['cover','taunt','doubleThrust','shieldBash'];
      const k=keys[row*2+col];
      if(!k)return;
      const lv=progress.gyouSkills[k]||0,max=gyouSkillMax(k);
      if(lv>=max){flashText='この技は最大強化です';flashTimer=1.4;return;}
      const cost=gyouSkillCost(k,lv);
      if((progress.gyouSP||0)<cost){flashText=`ジュウのSPが足りない（必要 ${cost}）`;flashTimer=1.6;return;}
      progress.gyouSP-=cost;progress.gyouSkills[k]=lv+1;saveProgress();saveGame();
      const names={cover:'かばう',taunt:'挑発',doubleThrust:(lv+1)>=2?'三段突き':'二段突き',shieldBash:'シールドバッシュ'};
      flashText=lv===0?`ジュウが「${names[k]}」を習得！`:`「${names[k]}」をLv.${lv+1}に強化！`;
  ...
  if(menuPage==='skill' && menuCharacter==='yuno' && yunoJoined && y>=245 && y<=455){
    ...
    if(col>=0&&col<4&&row>=0&&row<2){
      const keys=['heal','haste','wind','mpRegenAll','archery'];
      ...
      flashText=k==='haste'?(lv?'「追い風」をLv.2に強化！':'「追い風」を習得！'):k==='mpRegenAll'?(lv?'「魔力の風」をLv.2に強化！':'「魔力の風」を習得！'):k==='archery'?(lv?'「風纏三連射」に強化！':'「二連射」を習得！'):(lv===0?'ユーノが新しい風術を習得！':'風術を強化！');
      flashTimer=1.8;return;
    }
  }
  ...
  if(menuPage==='skill' && menuCharacter==='hero'){
    if(x>=690&&x<=915&&y>=360&&y<=452){
      ...
      if(ml>=2){flashText='水脈循環は最大強化です';flashTimer=1.4;return;}
      const cost=ml===0?2:3;
      ...
      flashText=ml===0?'パッシブ「水脈循環」を習得！':'「水脈循環」をLv.2に強化！';flashTimer=1.8;return;
    }
    ...
    const tryPebbleNode=(route,idx,cost,name)=>{
      ...
      if(route==='random'){progress.arpgSkills ||= {};progress.arpgSkills.hero ||= {};progress.arpgSkills.hero.iceShot=Math.max(progress.arpgSkills.hero.iceShot||1,Math.min(4,progress[key]+1));}
...
function pressAction(){
  ...
  if(scene==='title'){
    ...
    if(titleSelection===1){if(window.confirm('セーブ・進行状況を初期状態へリセットします。\n本当に実行しますか？'))startFromInitialState();return;}
...
function frame(now){
  ...
  else if(scene==='battle')arpgDrawBattle();
...
canvas.addEventListener('pointerdown',e=>{
  ...
  if(scene==='title'){
    ...
    // リセットは左下の小ボタンだけを判定対象にする。
    if(x>=24&&x<=232&&y>=466&&y<=514){titleSelection=1;pressAction();return;}
    if(progress.gameCleared){
      if(y>=290&&y<345){titleSelection=0;pressAction();return;}
      if(y>=395&&y<445){titleSelection=2;pressAction();return;}
      if(y>=445&&y<=500&&x>=260){titleSelection=3;pressAction();return;}
    }else{
      if(y>=316&&y<372){titleSelection=0;pressAction();return;}
      if(y>=428&&y<=472){titleSelection=2;pressAction();return;}
    }
    return;
  }
  if(scene==='sarubibiShop'){
    ...
    if(battleMenu==='main'){
      ...
      if(y>=385 && y<=500){
        if(isGyouTurn()){
          ...
          const modes=['cover','taunt','doubleThrust','shieldBash','grandGuard'];
          ...
          if(y>=370&&y<=432){
            ...
            else {battleMessage='MPは戦闘中に自然回復します。水脈循環で回復量を強化できます。';return;}
...
// ============================================================
const arpgButtons=document.getElementById('arpgButtons');
const arpgAttackBtn=document.getElementById('arpgAttack');
const arpgSkillABtn=document.getElementById('arpgSkillA');
const arpgSkillBBtn=document.getElementById('arpgSkillB');
const arpgSkillCBtn=document.getElementById('arpgSkillC');
const arpgAiBtn=document.getElementById('arpgAiBtn');
let arpg=null;
let arpgPanel=null;
...
function arpgEnsureProgress(){
  ...
  const defaults=arpgDefaultAI();
  for(const who of ['suzu','yuno','gyou']){
    if(!Array.isArray(progress.arpgAI[who]) || !progress.arpgAI[who].length)progress.arpgAI[who]=[...defaults[who]];
  }
...
function arpgRank(who,key){arpgEnsureProgress();if(who==='gyou'&&['cover','taunt','shieldBash','doubleThrust'].includes(key))return progress.gyouSkills?.[key]||0;return progress.arpgSkills?.[who]?.[key]||1;}
...
function arpgIceSlashCount(){
  // 九尾時も実際の高速移動は最大3回まで。3本の残像線それぞれを1撃分として扱う。
  return arpgIceSlashRank();
}
...
function arpgSpendSP(who,key){
  ...
  if(who==='gyou'&&key==='spear'){flashText='槍攻撃は基本技です';flashTimer=1.2;return;}
  if(who==='gyou'&&key==='ultimate'){flashText=progress.hiddenSkills?.gyou?'天地崩槍は習得済み':'天地崩槍はスキル画面でSP100で習得';flashTimer=1.5;return;}
  const max=who==='gyou'?gyouSkillMax(key):who==='hero'&&key==='iceSlash'?3:who==='hero'&&key==='iceShot'?4:5;
  if(lv>=max){flashText=`このスキルは最大Lv.${max}`;flashTimer=1.4;return;}
  // 主人公は旧試作の一律SP1を廃止し、段階が進むほど少し重くする。
  const cost=who==='gyou'?gyouSkillCost(key,lv):who==='hero'?Math.max(2,lv+1):1;
  if((progress[spk]||0)<cost){flashText=`SPが足りない（必要 ${cost}）`;flashTimer=1.4;return;}
  progress[spk]-=cost;
  if(who==='gyou')progress.gyouSkills[key]=lv+1;else progress.arpgSkills[who][key]=lv+1;
...
function arpgEnemySource(){
  ...
  return [{name:battle.enemyName||'敵',kind:battle.enemyKind||'pirate',hp:battle.enemyHP||1,maxHP:battle.enemyMaxHP||battle.enemyHP||1,id:battle.monsterId}];
}
function arpgPartyEnabled(who){
  // ARPGでは、物語上すでに加入している仲間は戦闘IDに関係なく参加する。
  // soloHero が明示された特殊戦だけ主人公単独にする。
  if(!battle || battle.soloHero)return false;
  if(who==='suzu') return !!(suzumaruJoined || suzumaruActive);
  if(who==='yuno') return !!yunoJoined;
...
function arpgCleanup(){
  arpgButtons?.classList.add('hidden'); actionBtn?.classList.remove('hidden');
  if(arpgPanel){arpgPanel.remove();arpgPanel=null;}
  arpg=null;
}
function arpgNearestEnemy(x,y,max=1e9){
  if(!arpg)return null; let best=null,bd=max;
  for(const e of arpg.enemies){if(e.src.hp<=0)continue;const d=Math.hypot(e.x-x,e.y-y);if(d<bd){bd=d;best=e;}}
  return best;
}
function arpgDamageEnemy(e,dmg,kind='hit'){
  if(!e||e.src.hp<=0)return; dmg=Math.max(1,Math.round(dmg));e.src.hp=Math.max(0,e.src.hp-dmg);e.flash=.12;
  if(!battle.enemies){battle.enemyHP=e.src.hp;} else if(battle.enemyHP!==undefined){const live=battle.enemies.find(x=>x.hp>0);battle.enemyHP=live?live.hp:0;}
  addDamagePopup(`-${dmg}`,e.x,e.y-34,kind==='ice'?'#c9f4ff':kind==='fire'?'#ffd0a8':'#ffffff');
  if(kind==='ice')sfx('ice');else if(kind==='fire')sfx('fire');else sfx('slash');
}
function arpgHealHero(amount,label='回復'){
  if(!battle)return;const before=battle.heroHP;battle.heroHP=Math.min(progress.maxHP,battle.heroHP+Math.round(amount));const got=battle.heroHP-before;
  if(got>0){addDamagePopup(`+${got}`,arpg.hero.x,arpg.hero.y-42,'#91f2d7');sfx('heal');arpg.message=`${label} +${got}`;arpg.messageT=1.4;}
}
function arpgRestoreMP(amount){
  const before=battle.heroMP;battle.heroMP=Math.min(progress.maxMP,battle.heroMP+Math.round(amount));const got=battle.heroMP-before;
  if(got>0){addDamagePopup(`MP+${got}`,arpg.hero.x,arpg.hero.y-58,'#8ed9ff');arpg.message=`ユーノ：MP回復 +${got}`;arpg.messageT=1.4;}
}
...
function arpgUseAttack(){
  ...
  const target=arpgNearestEnemy(arpg.hero.x,arpg.hero.y);
  let ang=arpg.hero.face<0?Math.PI:0;
  if(target){ang=Math.atan2(target.y-arpg.hero.y,target.x-arpg.hero.x);if(Math.abs(target.x-arpg.hero.x)>.1)arpg.hero.face=target.x>=arpg.hero.x?1:-1;}
  const e=target&&Math.hypot(target.x-arpg.hero.x,target.y-arpg.hero.y)<=92?target:null;
  if(e){
  ...
  arpg.fx.push({type:'daggerArc',x:arpg.hero.x,y:arpg.hero.y-8,angle:ang,age:0,life:.22});
  sfx('slash');
...
function arpgUseA(){
  ...
  const baseDist=count===1?124:count===2?108:96;
...
function arpgReleaseB(){
  ...
  if(maxed){
    ...
    const baseShots=lv<=1?1:lv===2?3:lv===3?5:7;
    const shotCount=baseShots+(progress.nineTailGear?2:0);
    // 狭い前方コーン内にランダム発射。ただし密集しすぎないよう最低限の角度差を取る。
    const halfCone=0.24; // 約±14度
    const offsets=[];
    for(let i=0;i<shotCount;i++){
      let off=shotCount===1?0:(Math.random()*2-1)*halfCone;
      for(let tries=0;tries<10&&offsets.some(v=>Math.abs(v-off)<0.035);tries++)off=(Math.random()*2-1)*halfCone;
      offsets.push(off);
    }
...
function arpgAllyTry(who,action){
  const a=arpg.allies[who],e=arpgNearestEnemy(a.x,a.y); if(!a||!e&& !['heal','mpHeal','speedBuff','ultimate','shield'].includes(action))return false;
  ...
  if(action==='windArea'&&a.cd.windArea<=0){
    const near=arpg.enemies.filter(x=>x.src.hp>0&&Math.hypot(x.x-a.x,x.y-a.y)<285);
    // 氷晶波の風版。敵が1体でもある程度の確率で使い、複数なら積極的に使う。
    if(near.length&&(near.length>=2||Math.random()<.48)){
      const maxR=210+lv*24,dmg=10+(progress.level||1)*.8+lv*3,kb=58+lv*18;
      arpg.fx.push({type:'windWave',x:a.x,y:a.y-8,age:0,life:.62,maxR,dmg,kb,hit:new Set()});
      a.cd.windArea=Math.max(3.6,6.5-lv*.25);arpg.message='ユーノの「風刃嵐」！';arpg.messageT=1.0;sfx('wind');return true;
    }
  }
  ...
  if(who==='yuno'&&action==='mpHeal'&&a.cd.mpHeal<=0){const slv=Math.max(1,progress.yunoSkills?.mpRegenAll||lv||1),mpRate=battle.heroMP/progress.maxMP;if(mpRate<.72&&(Math.random()<.52||mpRate<.38)){const amount=slv>=2?26:16;const before=battle.heroMP;battle.heroMP=Math.min(progress.maxMP,battle.heroMP+amount);const got=Math.round(battle.heroMP-before);if(got>0){addDamagePopup(`MP+${got}`,arpg.hero.x,arpg.hero.y-58,'#8ed9ff');arpg.message=`ユーノの「魔力の風」！ MP+${got}`;arpg.messageT=1.3;}a.cd.mpHeal=slv>=2?7.0:8.5;return got>0;}}
  if(who==='yuno'&&action==='speedBuff'&&a.cd.speedBuff<=0){const slv=Math.max(1,progress.yunoSkills?.haste||lv||1);if(arpg.partySpeedBuff<=1.0&&(Math.random()<.48||arpg.enemies.filter(x=>x.src.hp>0).length>=3)){arpg.partySpeedBuff=slv>=2?8.0:6.0;arpg.partySpeedMul=slv>=2?1.38:1.28;a.cd.speedBuff=slv>=2?12:14;arpg.message=`ユーノの「追い風」！ 全員の移動速度アップ！`;arpg.messageT=1.4;return true;}}
  ...
  if(action==='doubleThrust'&&a.cd.doubleThrust<=0&&!a.thrustSeq){const tlv=progress.gyouSkills?.doubleThrust||0;if(tlv>0&&Math.hypot(e.x-a.x,e.y-a.y)<175){a.thrustSeq={hits:tlv>=2?3:2,left:tlv>=2?3:2,timer:0,target:e,lv:tlv};a.cd.doubleThrust=tlv>=2?4.8:5.4;return true;}}
...
function arpgUpdateAlly(who,dt){
  if(!arpgPartyEnabled(who))return;
  const a=arpg.allies[who];
  for(const k in a.cd)a.cd[k]=Math.max(0,a.cd[k]-dt*1.18);
  if(a.shield)a.shield=Math.max(0,a.shield-dt);
  if(a.taunt)a.taunt=Math.max(0,a.taunt-dt);
  if(a.ultGuard)a.ultGuard=Math.max(0,a.ultGuard-dt);
  if(who==='gyou'&&a.thrustSeq){
    a.thrustSeq.timer-=dt;
    if(a.thrustSeq.timer<=0&&a.thrustSeq.left>0){
      let t=a.thrustSeq.target;if(!t||t.src.hp<=0)t=arpgNearestEnemy(a.x,a.y,190);
      if(t){const dx=t.x-a.x,dy=t.y-a.y,d=Math.max(1,Math.hypot(dx,dy));a.face=dx>=0?1:-1;arpgDamageEnemy(t,7+(progress.level||1)*.82+a.thrustSeq.lv*4);arpg.fx.push({type:'spearThrust',x:a.x,y:a.y-9,dx:dx/d,dy:dy/d,age:0,life:.19,long:90});sfx('spear');}
      a.thrustSeq.left--;a.thrustSeq.timer=.16;
    }
    if(a.thrustSeq.left<=0&&a.thrustSeq.timer<=0)a.thrustSeq=null;
  }

  const e=arpgNearestEnemy(a.x,a.y);
  const follow={suzu:[-58,58],yuno:[-92,-55],gyou:[-35,-62]}[who];
  let tx=arpg.hero.x+follow[0],ty=arpg.hero.y+follow[1],speed=150;

  if(e){
    const ex=e.x-a.x,ey=e.y-a.y,ed=Math.max(1,Math.hypot(ex,ey));
    const hx=e.x-arpg.hero.x,hy=e.y-arpg.hero.y,hd=Math.max(1,Math.hypot(hx,hy));
    if(who==='suzu'){
      // 前衛。敵へ踏み込み、剣と火炎斬りが届く距離を保つ。
      if(ed>92){tx=e.x-ex/ed*72;ty=e.y-ey/ed*72;speed=188;}
    }else if(who==='yuno'){
      // 後衛。主人公から離れすぎず、敵とは弓向けの間合いを取る。
      if(ed<145){tx=a.x-ex/ed*105;ty=a.y-ey/ed*105;speed=174;}
      else if(ed>245){tx=e.x-ex/ed*205;ty=e.y-ey/ed*205;speed=165;}
      else{
        a.strafeT=(a.strafeT||0)-dt;
        if(a.strafeT<=0){a.strafeT=.65+Math.random()*1.1;a.strafeDir=Math.random()<.5?-1:1;}
        tx=a.x+(-ey/ed)*(a.strafeDir||1)*26;ty=a.y+(ex/ed)*(a.strafeDir||1)*26;
      }
      if(Math.hypot(a.x-arpg.hero.x,a.y-arpg.hero.y)>235){tx=arpg.hero.x+follow[0];ty=arpg.hero.y+follow[1];}
    }else if(who==='gyou'){
      // 盾役。主人公と敵の間へ先回りし、危険が薄ければ槍の間合いへ。
      if(hd<230){tx=arpg.hero.x+hx/hd*48;ty=arpg.hero.y+hy/hd*48;speed=190;}
      else if(ed>118){tx=e.x-ex/ed*96;ty=e.y-ey/ed*96;speed=172;}
    }
  }
  ...
  a.think-=dt;
  if(a.think>0)return;
  a.think=.28+Math.random()*.62; // cooldown明け即使用にはせず、判断の間を作る
  if(Math.random()<.16)return;
  const order=progress.arpgAI?.[who]||arpgDefaultAI()[who];
  for(const act of order){if(arpgAllyTry(who,act))break;}
}
...
function arpgUpdateEnemy(e,dt){
  ...
  const g=arpg.allies.gyou,taunted=arpgPartyEnabled('gyou')&&g.taunt>0&&Math.hypot(e.x-g.x,e.y-g.y)<(g.tauntRadius||9999);
  const target=taunted?g:arpg.hero;
  ...
  if(e.src.kind==='dragon'){
    ...
    // 大型なので中心座標に広めの余白を取り、画面外へ押し出されないようにする。
    e.x=Math.max(125,Math.min(W-125,e.x));e.y=Math.max(155,Math.min(405,e.y));
    return;
  }
...
function arpgUpdate(dt){
  ...
  touchUI.classList.remove('hidden');actionBtn.classList.add('hidden');arpgButtons.classList.remove('hidden');
  if(arpg.messageT>0)arpg.messageT-=dt;if(arpg.hero.inv>0)arpg.hero.inv-=dt;for(const k in arpg.cd)arpg.cd[k]=Math.max(0,arpg.cd[k]-dt*1.18);
  ...
  for(const f of arpg.fx){
    ...
    if(f.type==='windWave'){
      ...
      for(const e of arpg.enemies){
        if(e.src.hp<=0||f.hit.has(e))continue;
        if(Math.hypot(e.x-f.x,e.y-f.y)<=r+22){
          f.hit.add(e);arpgDamageEnemy(e,f.dmg,'hit');
          const dx=e.x-f.x,dy=e.y-f.y,d=Math.max(1,Math.hypot(dx,dy));
          e.x=Math.max(35,Math.min(925,e.x+dx/d*f.kb));e.y=Math.max(105,Math.min(465,e.y+dy/d*f.kb));
          e.attackCd=Math.max(e.attackCd,.4);addDamagePopup('ノックバック',e.x,e.y-52,'#bdf6cf');
        }
      }
    }
  }
...
function arpgDrawSpearThrust(f){
  ...
  ctx.strokeStyle=f.ultimate?`rgba(255,226,124,${.25+a*.72})`:`rgba(235,241,230,${.2+a*.72})`;ctx.lineWidth=f.ultimate?8:5;ctx.beginPath();ctx.moveTo(5,0);ctx.lineTo(len,0);ctx.stroke();
  ctx.fillStyle=f.ultimate?'#ffe77d':'#dfe5dc';ctx.beginPath();ctx.moveTo(len+16,0);ctx.lineTo(len-2,-7);ctx.lineTo(len-2,7);ctx.closePath();ctx.fill();ctx.restore();
}
function arpgDrawShieldBash(f){
  const t=Math.min(1,f.age/f.life),r=28+70*t,a=1-t;ctx.save();ctx.globalAlpha=a;ctx.strokeStyle='#ffe59a';ctx.lineWidth=7;ctx.beginPath();ctx.arc(f.x,f.y,r,-.8,.8);ctx.stroke();ctx.restore();
}
function arpgDrawArrow(p){
  const ang=Math.atan2(p.vy,p.vx);ctx.save();ctx.translate(p.x,p.y);ctx.rotate(ang);ctx.strokeStyle='#6a4b2f';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-18,0);ctx.lineTo(12,0);ctx.stroke();ctx.fillStyle='#e9eef0';ctx.beginPath();ctx.moveTo(18,0);ctx.lineTo(8,-5);ctx.lineTo(8,5);ctx.closePath();ctx.fill();ctx.strokeStyle='#d8f1e9';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(-15,0);ctx.lineTo(-22,-6);ctx.moveTo(-15,0);ctx.lineTo(-22,6);ctx.stroke();ctx.restore();
}
...
function arpgDrawWindWave(f){
  const t=Math.min(1,f.age/f.life),r=f.maxR*t,alpha=Math.max(0,1-t);
  ctx.save();
  ctx.globalAlpha=.20*alpha;ctx.fillStyle='#b9f2c8';ctx.beginPath();ctx.arc(f.x,f.y,r,0,Math.PI*2);ctx.fill();
  ctx.globalAlpha=.88*alpha;ctx.strokeStyle='#d8ffe1';ctx.lineWidth=9;ctx.beginPath();ctx.arc(f.x,f.y,r,0,Math.PI*2);ctx.stroke();
  ctx.globalAlpha=.60*alpha;ctx.strokeStyle='#5fd58a';ctx.lineWidth=4;ctx.beginPath();ctx.arc(f.x,f.y,Math.max(0,r-17),0,Math.PI*2);ctx.stroke();
  ctx.restore();
}
...
function arpgBuildPanel(){
  ...
  const names={suzu:'スズマル',yuno:'ユーノ',gyou:'ジュウ'}, labels={sword:'剣攻撃',fireSlash:'火炎斬り',fireArea:'範囲炎魔法',bow:'弓攻撃',windArea:'風範囲魔法',heal:'主人公回復',mpHeal:'主人公MP回復',speedBuff:'追い風（移動速度UP）',ultimate:'天風の祝福',spear:'槍攻撃',cover:'かばう',taunt:'挑発',shieldBash:'シールドバッシュ',doubleThrust:'二段突き / 三段突き',ultimate:'天地崩槍'};
...
function arpgMoveAI(spec,dir){const [who,idxs]=spec.split(':'),i=+idxs,a=progress.arpgAI[who];const j=i+dir;if(j<0||j>=a.length)return;[a[i],a[j]]=[a[j],a[i]];saveProgress();saveGame();arpgBuildPanel();}

arpgAttackBtn?.addEventListener('pointerdown',e=>{e.preventDefault();arpgUseAttack();});
arpgSkillABtn?.addEventListener('pointerdown',e=>{e.preventDefault();arpgUseA();});
arpgSkillBBtn?.addEventListener('pointerdown',e=>{e.preventDefault();arpgBeginB();});
arpgSkillBBtn?.addEventListener('pointerup',e=>{e.preventDefault();arpgReleaseB();});
arpgSkillBBtn?.addEventListener('pointercancel',()=>arpgReleaseB());
arpgSkillCBtn?.addEventListener('pointerdown',e=>{e.preventDefault();arpgUseC();});
arpgAiBtn?.addEventListener('pointerdown',e=>{e.preventDefault();if(!arpgPanel)arpgBuildPanel();});
addEventListener('keydown',e=>{if(scene!=='battle'||!battle)return;if(e.repeat&&e.key.toLowerCase()!=='j')return;const k=e.key.toLowerCase();if(k==='j')arpgUseAttack();else if(k==='k')arpgUseA();else if(k==='l'&&!arpg?.charging)arpgBeginB();else if(k==='h')arpgUseC();else if(k==='p'&&!arpgPanel)arpgBuildPanel();});
addEventListener('keyup',e=>{if(scene==='battle'&&e.key.toLowerCase()==='l')arpgReleaseB();});
})();